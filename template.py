...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            overlap_ratio = len(chunk_tokens & expected_tokens) / expected_len
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"Answer: {answer}\n"
            f"Rubric:\n{rubric_lines}\n\n"
            "Score each criterion from 0.0 to 1.0 and return a JSON object "
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        entry_averages = [_avg(entry) for entry in scores_batch]
        overall_avg = sum(entry_averages) / len(entry_averages)

        # Positional bias: entries earlier in the batch (assumed presented
        # first) consistently score higher than later ones.
        positional_bias = False
        if len(entry_averages) >= 2:
            midpoint = len(entry_averages) // 2
            first_half = entry_averages[:midpoint] or entry_averages[:1]
            second_half = entry_averages[midpoint:] or entry_averages[-1:]
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        """
        low_faithfulness = failure.faithfulness < 0.5
        low_relevance = failure.relevance < 0.5
        low_completeness = failure.completeness < 0.5
        low_count = sum([low_faithfulness, low_relevance, low_completeness])
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant"):
            suggestions.append(
                "Add prompt clarification steps to keep answers on-topic"
            )
        ...
        if categories.get("off_topic"):
            suggestions.append(
                "Improve intent detection to route questions to the right context"
            )
        ...
        suggestions.append(
            "Expand the golden dataset with more edge cases to catch regressions earlier"
        )
